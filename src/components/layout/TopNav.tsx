...
export default function TopNav() {
  ...
  return (
    <div className={`${APP_CONTENT_MAX} mx-auto px-4 pt-6 sm:px-6 xl:px-8`}>
      <div className="grid gap-6 lg:grid-cols-[84px_minmax(0,1fr)]">
        <aside className="sidebar-pill hidden h-[calc(100vh-48px)] min-h-[760px] flex-col justify-between p-4 lg:flex">
            ...
            {items.map((item) => (
              <Link
                ...
                className={[
                  ...
                  isActive(item.href)
                    ? 'border-slate-200 bg-slate-100 text-[var(--text-primary)] shadow-inner'
          ...
          <div className="space-y-3">
            ...
            <button
              type="button"
              onClick={logout}
              className="flex h-14 w-14 items-center justify-center rounded-2xl border border-transparent text-slate-500 transition hover:bg-slate-50"
              title="Sign out"
            >
        ...
        <div className="space-y-6">
          <header className="soft-card flex min-h-[84px] items-center justify-between px-6">
            <div>
              <h1 className="serif-display text-[28px] font-semibold tracking-tight text-[var(--text-primary)]">
                Job Seeker Dashboard
              </h1>
              <p className="mt-1 text-sm text-[var(--text-secondary)]">
                Premium SaaS minimalism with real profile-driven content
              </p>
            </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <Link
                href="/profile"
                className="flex h-11 min-w-[44px] items-center justify-center rounded-2xl border border-slate-200 bg-white px-4 text-sm font-medium text-slate-700"
              >
                {name || 'Profile'}
              </Link>
            </div>
