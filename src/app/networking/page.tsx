    ...
    <div className="max-w-3xl">
      ...
      <div className="mt-5 flex gap-2">
        {[{id:'recruiter' as Tab,label:'Find Recruiter',icon:UserCheck},{id:'email' as Tab,label:'Cold Email',icon:Mail},{id:'referral' as Tab,label:'Referral Tips',icon:Users}].map(t => (
          <button key={t.id} onClick={() => { setTab(t.id); setResult(null); }} className={`flex items-center gap-2 rounded-2xl px-4 py-2.5 text-sm font-semibold transition-all ${tab === t.id ? 'bg-[#007886]/10 text-[#00daf3] border border-[#30d158]/20 shadow-xs' : 'text-[#8e90a2] hover:bg-[var(--surface-2)] border border-transparent'}`}>
      ...
      <div className="mt-5 surface p-6">
        <div className="grid grid-cols-2 gap-4">
          <div><label className="block text-xs font-semibold text-[#8e90a2] mb-1.5">Company</label><input type="text" value={company} onChange={e => setCompany(e.target.value)} placeholder="e.g. Google, Meta" className="input-hig" /></div>
          <div><label className="block text-xs font-semibold text-[#8e90a2] mb-1.5">Job Title</label><input type="text" value={jobTitle} onChange={e => setJobTitle(e.target.value)} placeholder="e.g. AI Engineer" className="input-hig" /></div>
        </div>
        {tab === 'email' && <div className="mt-4"><label className="block text-xs font-semibold text-[#8e90a2] mb-1.5">Job Description (optional)</label><textarea rows={4} value={jd} onChange={e => setJd(e.target.value)} placeholder="Paste JD for personalized email..." className="input-hig" /></div>}
      ...
      {result && (
        <div className="mt-5 surface p-6">
              ...
              <div className="space-y-3">
                {result.recruiters.map((r: any, i: number) => (
                  <div key={i} className="flex items-center gap-3 bg-[var(--surface-1)] rounded-2xl p-3">
                    ...
                    {r.linkedin_search && <a href={r.linkedin_search} target="_blank" rel="noopener noreferrer" className="pill bg-[#3c59fd]/10 text-[#bbc3ff] border border-[#0a84ff]/20 gap-1 cursor-pointer hover:bg-blue-100"><Linkedin className="h-3 w-3" />Find</a>}
                  </div>
                ))}
              </div>
          ...
          {tab === 'email' && result.email && (
            <div>
              <div className="flex items-center justify-between mb-3"><h3 className="text-sm font-bold text-[#c4c5d9]">Cold Outreach Email</h3><button onClick={() => copy(result.subject + '\n\n' + result.email)} className="pill bg-[var(--surface-1)] text-[#c4c5d9] border border-[var(--separator)] gap-1 cursor-pointer hover:bg-[var(--surface-2)]"><Copy className="h-3 w-3" />Copy</button></div>
              ...
              <div className="bg-[var(--surface-1)] rounded-2xl p-4 text-sm text-[#c4c5d9] whitespace-pre-wrap leading-relaxed">{result.email}</div>
            </div>
          )}
            ...
            <div>
              ...
              <div className="space-y-2">
                {result.strategies.map((s: any, i: number) => (
                  <div key={i} className="bg-[var(--surface-1)] rounded-2xl p-3"><p className="text-sm font-semibold text-[#c4c5d9]">{s.title}</p><p className="text-xs text-[#8e90a2] mt-1">{s.description}</p></div>
                ))}
              </div>
            </div>
          )}
