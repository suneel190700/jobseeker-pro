...
export default function JobsPage() {
  ...
  return(<div>
    ...
    />
    {titles.length>1&&<div className="mt-2 flex flex-wrap gap-2">{titles.map(t=>(<button key={t} type="button" onClick={()=>handleSearch(undefined,t)} className={`pill cursor-pointer transition-all border px-4 py-1.5 ${query===t&&searched?'bg-[var(--accent-dim)] text-[var(--accent)] border-[var(--accent-dim-strong)]':'bg-[var(--surface-1)] text-[var(--text-secondary)] border-[var(--separator)] hover:border-[var(--accent-dim-strong)] hover:text-[var(--accent)]'}`}>{t}</button>))}</div>}

    <form onSubmit={e=>handleSearch(e)} className="mt-5">
      <div className="flex gap-3 surface p-2.5">
        <div className="relative flex-1"><Search className="absolute left-3.5 top-1/2 h-4 w-4 -translate-y-1/2 text-[var(--text-tertiary)]"/><input type="text" value={query} onChange={e=>setQuery(e.target.value)} placeholder="Job title, skills, or company" className="w-full rounded-[var(--radius-lg)] bg-[var(--surface-1)] border border-transparent py-2.5 pl-10 pr-4 text-sm text-[var(--text-primary)] placeholder:text-[var(--text-tertiary)] focus:border-[var(--accent-dim-strong)] focus:ring-2 focus:ring-[var(--accent-dim)] focus:outline-none transition"/></div>
        <div className="relative w-44 min-w-0 sm:min-w-[11rem]"><MapPin className="absolute left-3.5 top-1/2 h-4 w-4 -translate-y-1/2 text-[var(--text-tertiary)]"/><input type="text" value={location} onChange={e=>setLocation(e.target.value)} placeholder="City, state" className="w-full rounded-[var(--radius-lg)] bg-[var(--surface-1)] border border-transparent py-2.5 pl-10 pr-4 text-sm text-[var(--text-primary)] placeholder:text-[var(--text-tertiary)] focus:border-[var(--accent-dim-strong)] focus:ring-2 focus:ring-[var(--accent-dim)] focus:outline-none transition"/></div>
      ...
      {showFilters&&<div className="mt-2 flex flex-wrap items-center gap-3 surface p-3.5">
        {[{l:'Remote',v:remoteFilter,s:setRemoteFilter,o:[['','Any'],['remote','Remote'],['onsite','On-site']]},{l:'Type',v:employmentType,s:setEmploymentType,o:[['','Any'],['fulltime','Full-time'],['contract','Contract'],['parttime','Part-time']]},{l:'Level',v:experienceLevel,s:setExperienceLevel,o:[['','Any'],['Entry Level','Entry'],['Mid Level','Mid'],['Senior','Senior'],['Lead','Lead']]},{l:'Posted',v:datePosted,s:setDatePosted,o:[['','Any'],['today','Today'],['3days','3 days'],['week','Week'],['month','Month']]}].map(f=>(
          <div key={f.l}><label className="block text-[10px] font-bold text-[var(--text-tertiary)] uppercase mb-1">{f.l}</label><select value={f.v} onChange={e=>f.s(e.target.value)} className="rounded-[var(--radius-md)] border border-[var(--separator)] bg-[var(--surface-1)] px-3 py-1.5 text-xs text-[var(--text-secondary)]">{f.o.map(([val,label])=><option key={val} value={val}>{label}</option>)}</select></div>
        ))}
    ...
    <div className="mt-5 flex gap-5">
      <div className="flex-1 space-y-2">
        ...
        {!loading&&jobs.map(job=>{const saved=isSaved(job);const sc=scores[job.id];const scoring=scoringId===job.id;return(
          <div key={job.id} onClick={()=>setSelectedJob(job)} className={`surface surface-h p-4 cursor-pointer border border-[var(--separator)] ${selectedJob?.id===job.id?'border-[var(--accent-dim-strong)] shadow-glow-sm ring-1 ring-[var(--accent)]/20':''}`}>
            <div className="flex items-start justify-between">
              ...
              <div className="flex flex-col items-end gap-1.5 ml-3">
                ...
                {!sc&&profile&&<button type="button" onClick={e=>{e.stopPropagation();scoreJob(job);}} disabled={scoring} className="pill bg-[var(--surface-1)] text-[var(--text-secondary)] border border-[var(--separator)] hover:bg-[var(--accent-dim)] hover:text-[var(--accent)] hover:border-[var(--accent-dim-strong)] transition cursor-pointer gap-1 px-2.5 py-1">{scoring?<Loader2 className="h-3 w-3 animate-spin"/>:<Target className="h-3 w-3"/>}{scoring?'...':'Score'}</button>}
