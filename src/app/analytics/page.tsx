...
export default function AnalyticsPage() {
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold text-slate-200 tracking-tight">Analytics</h1>
      <p className="mt-1 text-sm text-slate-600">Application funnel, timing insights & resume testing</p>

      {/* Funnel */}
      <div className="mt-6 glass p-6">
        <h2 className="text-sm font-bold text-slate-300 flex items-center gap-2"><BarChart3 className="h-4 w-4 text-emerald-400" />Application Funnel</h2>
          ...
          {stageCounts.map((s, i) => {
            ...
            return (
              <div key={s.stage} className="flex-1 flex flex-col items-center gap-1.5">
                ...
                <span className="text-[10px] font-semibold text-slate-600 capitalize">{s.stage}</span>
              </div>
            );
          })}
        </div>
        {applied > 0 && (
          <div className="mt-5 grid grid-cols-3 gap-3">
            {[{label:'Screen Rate',mine:myRates.screening,bench:benchmarks.screening},{label:'Interview Rate',mine:myRates.interview,bench:benchmarks.interview},{label:'Offer Rate',mine:myRates.offer,bench:benchmarks.offer}].map(r => (
              <div key={r.label} className="bg-[rgba(255,255,255,0.02)] rounded-xl p-3">
                <p className="text-[10px] font-semibold text-slate-600 uppercase">{r.label}</p>
                <div className="flex items-baseline gap-2 mt-1">
                  <span className={`text-xl font-bold ${r.mine >= r.bench ? 'text-emerald-400' : 'text-amber-400'}`}>{r.mine}%</span>
                  <span className="text-xs text-slate-600">vs {r.bench}% avg</span>
                </div>
              </div>
            ))}
          </div>
        )}
        {applied === 0 && <p className="mt-4 text-sm text-slate-600 text-center py-4">Start tracking applications to see your funnel analytics</p>}
      </div>

      <div className="mt-5 grid grid-cols-2 gap-5">
        {/* Timing Optimizer */}
        <div className="glass p-6">
          <h2 className="text-sm font-bold text-slate-300 flex items-center gap-2"><Clock className="h-4 w-4 text-violet-500" />Best Time to Apply</h2>
          <p className="text-xs text-slate-600 mt-1">Based on recruiter activity patterns</p>
          <div className="mt-4 space-y-2">
            {bestTimes.map(t => (
              <div key={t.day} className="flex items-center gap-3">
                <span className="text-xs font-semibold text-slate-400 w-16">{t.day.slice(0, 3)}</span>
                <div className="flex-1 bg-[rgba(255,255,255,0.04)] rounded-full h-5 overflow-hidden">
                ...
                <span className="text-[10px] text-slate-500 w-20 text-right">{t.time}</span>
              </div>
            ))}
          </div>
        ...
        {/* Resume A/B Testing */}
        <div className="glass p-6">
          <h2 className="text-sm font-bold text-slate-300 flex items-center gap-2"><FileText className="h-4 w-4 text-blue-500" />Resume A/B Testing</h2>
          <p className="text-xs text-slate-600 mt-1">Track which resume version performs better</p>
          <div className="mt-4 bg-[rgba(255,255,255,0.02)] rounded-xl p-4 text-center">
            <Zap className="h-8 w-8 text-slate-600 mx-auto" />
            <p className="mt-2 text-sm font-semibold text-slate-400">Coming Soon</p>
            <p className="text-xs text-slate-600 mt-1">Generate 2 resume versions for a job, track which gets more callbacks. Assign versions A/B in tracker.</p>
            <div className="mt-3 flex gap-2 justify-center">
              <div className="badge-dark bg-blue-500/10 text-blue-400 border border-blue-500/20">Version A</div>
              <span className="text-xs text-slate-600 self-center">vs</span>
              <div className="badge-dark bg-purple-500/10 text-purple-400 border border-purple-500/20">Version B</div>
            </div>
          </div>
        </div>
      </div>
