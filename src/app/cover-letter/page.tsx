...
export default function CoverLetterPage() {
  ...
  return (
    <div className="max-w-4xl">
      ...
      <p className="mt-1 text-sm text-dark-200">Generate a tailored cover letter for any job.</p>
      ...
      <div className="mt-8 grid gap-6 lg:grid-cols-2">
        <div className="space-y-4">
          ...
          <div><label className="block text-xs font-medium text-dark-100 mb-1">Tone</label>
            <div className="flex gap-2">{[{ v: 'professional', l: 'Professional' }, { v: 'formal', l: 'Formal' }, { v: 'enthusiastic', l: 'Enthusiastic' }].map((t) => (<button key={t.v} onClick={() => setTone(t.v)} className={['rounded-lg border px-3 py-1.5 text-xs font-medium transition', tone === t.v ? 'border-brand-500 bg-indigo-500/10 text-brand-300' : 'border-dark-500 text-dark-200'].join(' ')}>{t.l}</button>))}</div>
          </div>
          <div><label className="block text-xs font-medium text-dark-100 mb-1">Job Description</label><textarea rows={10} value={jobDescription} onChange={(e) => setJobDescription(e.target.value)} placeholder="Paste full JD..." className="w-full rounded-lg border border-dark-400 bg-dark-700 px-3 py-2.5 text-sm text-white focus:border-brand-400 focus:outline-none focus:ring-1 focus:ring-brand-400" /></div>
        ...
        <div className="rounded-xl border border-dark-500 p-6 min-h-[500px]">
          {!result ? (
            <div className="flex h-full items-center justify-center text-sm text-dark-300">{generating ? <Loader2 className="h-6 w-6 animate-spin text-brand-400" /> : 'Cover letter will appear here.'}</div>
          ) : (
            <div className="space-y-4">
              {result.subject_line && (<div className="rounded-lg bg-dark-700 p-3"><p className="text-[10px] font-medium text-dark-300 uppercase">Email Subject</p><p className="text-sm font-medium text-dark-50 mt-0.5">{result.subject_line}</p></div>)}
              <div className="text-sm text-dark-100 leading-relaxed whitespace-pre-wrap">{result.cover_letter}</div>
              <div className="flex gap-2 pt-2 border-t">
                <button onClick={handleCopy} className="flex-1 rounded-lg border border-dark-400 px-4 py-2 text-sm font-medium text-dark-100 hover:bg-dark-700 transition flex items-center justify-center gap-2">
