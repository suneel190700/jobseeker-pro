...
export default function CoverLetterPage() {
  ...
  useEffect(() => {
    ...
    const opt = sessionStorage.getItem('optimized_resume');
    if (opt) setOptResume(opt);
  }, []);

  const handleGenerate = async () => {
    if (!profile?.text && !optResume) { toast.error('Upload resume in Profile first.'); return; }
    if (!jobDescription.trim()) { toast.error('Paste a job description.'); return; }
    setGenerating(true); setError(''); setResult(null);
    try {
      const resumeText = optResume || profile?.text || '';
      const res = await fetch('/api/cover-letter', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ resumeText, jobDescription, jobTitle, companyName, tone }) });
