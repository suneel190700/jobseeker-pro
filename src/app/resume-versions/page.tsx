...
import { FileText, Trash2, Download, Eye, Clock, Target, TrendingUp } from 'lucide-react';
...
export default function ResumeVersionsPage() {
  ...
  const downloadVersion = async (v: SavedVersion) => {
    try {
      ...
      const a = document.createElement('a'); a.href = url; a.download = `resume_${v.label.replace(/\s+/g, '_')}.docx`;
  ...
  return (
    <div>
      ...
      <p className="mt-1 text-sm text-slate-500">All your tailored resumes in one place. {versions.length} version{versions.length !== 1 ? 's' : ''} saved.</p>

      {versions.length === 0 ? (
        <div className="mt-12 text-center py-16">
          <FileText className="h-12 w-12 text-slate-300 mx-auto" />
          <p className="mt-3 text-sm text-slate-400">No saved versions yet.</p>
          <p className="text-xs text-slate-400 mt-1">Generate optimized resumes in the Resume Optimizer — they'll appear here.</p>
              ...
              <div key={v.id} onClick={() => setSelected(v)} className={['rounded-xl border p-4 cursor-pointer transition', selected?.id === v.id ? 'border-brand-300 bg-brand-50/30 shadow-sm' : 'border-slate-200 hover:border-slate-300 hover:shadow-sm'].join(' ')}>
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    <p className="text-xs text-slate-500 mt-0.5">{v.role} at {v.company}</p>
                    <div className="mt-2 flex items-center gap-3 text-xs text-slate-400">
                      <span className="flex items-center gap-1"><Target className="h-3 w-3" /><span className={v.score >= 80 ? 'text-green-600 font-medium' : v.score >= 60 ? 'text-amber-600' : 'text-red-600'}>{v.score}% ATS</span></span>
            ...
            <div className="hidden lg:block w-[450px] flex-shrink-0">
              <div className="sticky top-0 rounded-xl border border-slate-200 p-6 max-h-[calc(100vh-200px)] overflow-y-auto">
                <div className="flex items-center justify-between mb-4">
                  <h2 className="text-sm font-semibold text-slate-700">Preview: {selected.label}</h2>
                  <span className={['rounded-full px-2 py-0.5 text-xs font-bold', selected.score >= 80 ? 'bg-green-100 text-green-700' : 'bg-amber-100 text-amber-700'].join(' ')}>{selected.score}%</span>
                </div>
                <div className="text-center border-b pb-3 mb-3"><p className="text-base font-bold text-slate-900">{selected.resume.name}</p><p className="text-[10px] text-slate-400">{[selected.resume.email, selected.resume.phone, selected.resume.location].filter(Boolean).join(' | ')}</p></div>
                ...
                {selected.resume.skills_grouped && <div className="mb-3"><p className="text-[10px] font-bold text-slate-500 uppercase">Skills</p>{Object.entries(selected.resume.skills_grouped).map(([cat, skills]) => (<p key={cat} className="text-xs text-slate-600 mt-0.5"><span className="font-medium">{cat}:</span> {(skills as string[]).join(', ')}</p>))}</div>}
                {selected.resume.experience?.map((exp: any, i: number) => (
                  <div key={i} className="mb-3"><p className="text-xs font-bold text-slate-800">{exp.title} — {exp.company}</p><p className="text-[10px] text-slate-400">{exp.dates}</p>{exp.bullets?.slice(0, 3).map((b: string, j: number) => (<p key={j} className="text-[10px] text-slate-500 mt-0.5">- {b}</p>))}{(exp.bullets?.length || 0) > 3 && <p className="text-[10px] text-slate-300">+{exp.bullets.length - 3} more</p>}</div>
                ))}
              </div>
            </div>
          )}
