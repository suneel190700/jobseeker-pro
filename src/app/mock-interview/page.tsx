...
export default function MockInterviewPage() {
  ...
  if(!started)return(
    <div className="max-w-2xl mx-auto">
      ...
      <p className="mt-1 text-sm text-[#8e90a2]">Practice with voice, get scored on content & delivery</p>
      <div className="mt-6 surface p-6 space-y-4">
        <div className="flex gap-2 mb-2">
          {(['practice','real'] as const).map(m=>(
            <button key={m} onClick={()=>setMode(m)} className={`flex-1 rounded-[16px] py-2.5 text-sm font-semibold transition-all ${mode===m?'bg-[#007886]/10 text-[#00daf3] border border-emerald-500/30':'bg-[var(--surface-2)] text-[#8e90a2] border border-[var(--separator)] hover:text-[#c4c5d9]'}`}>
              {m==='practice'?'🎓 Practice Mode':'🎯 Real Mode'}
            </button>
          ))}
        </div>
        <p className="text-xs text-[#434656]">{mode==='practice'?'Shows suggested answers & key points for each question':'No hints — simulates a real interview'}</p>
  ...
  return(
    <div className="flex flex-col h-[calc(100vh-100px)]">
      <div className="flex items-center justify-between mb-3">
        <div><h1 className="text-base font-bold text-white">{role||'Mock Interview'}{company?` — ${company}`:''}</h1><p className="text-xs text-[#434656]">Q{qNum} • {mode} mode{avg>0?` • Avg: ${avg}/10`:''}</p></div>
        <div className="flex gap-2">
          {mode==='practice'&&<button onClick={()=>setShowHints(!showHints)} className={`btn-gray text-xs px-3 py-1.5 flex items-center gap-1 ${showHints?'text-[#00daf3]':'text-[#8e90a2]'}`}>{showHints?<Eye className="h-3 w-3"/>:<EyeOff className="h-3 w-3"/>}Hints</button>}
          {avg>0&&<div className={`pill border px-2.5 py-1 ${avg>=7?'bg-[#007886]/10 text-[#00daf3] border-[#30d158]/20':avg>=5?'bg-[#5203d5]/10 text-[#cdbdff] border-[#ff9f0a]/20':'bg-[#93000a]/10 text-[#ffb4ab] border-[#ff453a]/20'}`}><Star className="h-3 w-3 mr-1"/>{avg}/10</div>}
        ...
        {msgs.map((m,i)=>(
          <div key={i}>
            <div className={`flex ${m.role==='you'?'justify-end':'justify-start'}`}>
              <div className={`max-w-[75%] rounded-2xl px-4 py-3 text-sm leading-relaxed ${m.role==='interviewer'?'bg-[var(--surface-2)] border border-[var(--separator)] text-[#c4c5d9]':m.role==='you'?'bg-emerald-600 text-white':m.role==='feedback'?'bg-[var(--surface-2)] border border-[var(--separator)]/50 text-[#c4c5d9]':'bg-[var(--surface-1)] text-[#8e90a2] text-xs'}`}>
                {m.role==='feedback'&&m.score!=null&&<div className={`pill mb-2 ${m.score>=7?'bg-[#007886]/10 text-[#00daf3]':m.score>=5?'bg-[#5203d5]/10 text-[#cdbdff]':'bg-[#93000a]/10 text-[#ffb4ab]'}`}><Star className="h-3 w-3 mr-1"/>{m.score}/10</div>}
                {m.text}
              </div>
            </div>
            {/* Filler data */}
            {m.fillerData&&m.fillerData.count>0&&(
              <div className="flex justify-end mt-1"><div className={`pill gap-1 ${m.fillerData.count<=2?'bg-[#007886]/10 text-[#00daf3]':'bg-[#5203d5]/10 text-[#cdbdff]'}`}>
            ...
            {m.role==='interviewer'&&mode==='practice'&&showHints&&(m.suggestion||m.keyPoints)&&(
              <div className="mt-2 ml-0 max-w-[75%] bg-emerald-500/5 border border-emerald-500/10 rounded-[16px] p-3">
                <div className="flex items-center gap-1 mb-1.5"><Lightbulb className="h-3 w-3 text-[#00daf3]"/><span className="text-[10px] font-bold uppercase tracking-wider text-[#00daf3]">Suggested Answer</span></div>
                {m.keyPoints&&<div className="flex flex-wrap gap-1 mb-2">{m.keyPoints.map((k,j)=><span key={j} className="pill bg-[#007886]/10 text-emerald-300 text-[10px]">{k}</span>)}</div>}
                {m.suggestion&&<p className="text-xs text-[#8e90a2] leading-relaxed">{m.suggestion}</p>}
              </div>
            )}
          </div>
        ))}
        {loading&&<div className="flex justify-start"><div className="bg-[var(--surface-2)] border border-[var(--separator)] rounded-2xl px-4 py-3"><Loader2 className="h-4 w-4 animate-spin text-[#00daf3]"/></div></div>}
      </div>

      {/* Recording indicator */}
      {recording&&<div className="mb-2 bg-[#93000a]/10 border border-[#ff453a]/20 rounded-[16px] p-3 flex items-center gap-2"><div className="h-2 w-2 rounded-full bg-red-500 animate-pulse"/><span className="text-xs text-[#ffb4ab] font-medium">Recording...</span><span className="text-xs text-[#434656] flex-1">{transcript||'Listening...'}</span><button onClick={stopRecording} className="pill bg-red-500/20 text-[#ffb4ab] border border-red-500/30 cursor-pointer hover:bg-red-500/30">Stop</button></div>}

      <div className="flex gap-2">
        <button onClick={recording?stopRecording:startRecording} className={`rounded-[16px] p-2.5 transition-all ${recording?'bg-red-500/20 text-[#ffb4ab] border border-red-500/30':'bg-[var(--surface-2)] text-[#8e90a2] border border-[var(--separator)] hover:text-[#00daf3] hover:border-emerald-500/30'}`}>
