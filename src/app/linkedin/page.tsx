...
export default function LinkedInPage() {
  ...
  return (
    <div className="max-w-4xl">
      <h1 className="text-2xl font-bold text-[#e1e2eb]">LinkedIn Optimizer</h1>
      <p className="mt-1 text-sm text-[#8e90a2]">Generate an optimized headline, about section, and skills list for your LinkedIn profile.</p>

      <div className="mt-8">
        {!profile ? (
          <div className="rounded-2xl bg-[#5203d5]/10 border border-[#ff9f0a]/20 p-4 text-sm text-[#cdbdff]">Upload your base resume in <a href="/profile" className="font-medium underline">Profile</a> first.</div>
        ) : (
          <div className="space-y-4">
            ...
            <button onClick={handleGenerate} disabled={generating} className="rounded-2xl bg-blue-600 px-6 py-3 text-sm font-semibold text-[#e1e2eb] hover:bg-blue-700 disabled:opacity-50 transition flex items-center gap-2">
        ...
        {result && (
          <div className="mt-8 space-y-6">
            ...
            <div className="rounded-2xl border border-[var(--separator)] p-5">
              ...
              <div className="rounded-2xl bg-[#3c59fd]/10 border border-blue-100 p-4">
                <p className="text-sm font-medium text-blue-900">{result.headline}</p>
              </div>
              <p className="mt-1 text-[10px] text-[#434656]">{result.headline?.length || 0}/220 characters</p>
            </div>
            ...
            <div className="rounded-2xl border border-[var(--separator)] p-5">
              ...
              <div className="flex flex-wrap gap-2">
                {result.skills?.map((s: string) => (<span key={s} className="rounded-full bg-[#3c59fd]/10 border border-[#0a84ff]/20 px-3 py-1 text-xs text-blue-700">{s}</span>))}
              </div>
            </div>

            {/* Tips */}
            {result.tips?.length > 0 && (
              <div className="rounded-2xl border border-[#ff9f0a]/20 bg-[#5203d5]/10/50 p-5">
                <h2 className="text-base font-semibold text-[#e1e2eb] flex items-center gap-2"><Lightbulb className="h-5 w-5 text-amber-500" /> Additional Tips</h2>
                <div className="mt-3 space-y-2">
                  {result.tips.map((tip: string, i: number) => (
                    <div key={i} className="flex items-start gap-2 text-sm text-[#c4c5d9]"><span className="text-amber-500 font-bold">{i + 1}.</span>{tip}</div>
                  ))}
                </div>
              </div>
            )}
