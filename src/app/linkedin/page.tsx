...
export default function LinkedInPage() {
  ...
  return (
    <div className="max-w-4xl">
      ...
      <p className="mt-1 text-sm text-gray-500">Generate an optimized headline, about section, and skills list for your LinkedIn profile.</p>
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between rounded-lg bg-green-50 border border-green-100 p-3">
              <div className="flex items-center gap-2 text-xs text-green-700"><CheckCircle className="h-4 w-4" /> Using: {profile.fileName}
                {titles.length > 0 && <span className="text-green-600">| Targeting: {titles.join(', ')}</span>}
              </div>
            </div>
          ...
          <div className="mt-8 space-y-6">
            ...
            <div className="rounded-xl border border-gray-200 p-5">
              <div className="flex items-center justify-between mb-3">
                ...
                <button onClick={() => copyField(result.headline, 'Headline')} className="flex items-center gap-1 text-xs text-gray-400 hover:text-gray-600 transition">
                  {copiedField === 'Headline' ? <CheckCircle className="h-3.5 w-3.5 text-green-600" /> : <Copy className="h-3.5 w-3.5" />} Copy
                </button>
              </div>
              <div className="rounded-lg bg-blue-50 border border-blue-100 p-4">
                <p className="text-sm font-medium text-blue-900">{result.headline}</p>
              </div>
              <p className="mt-1 text-[10px] text-gray-400">{result.headline?.length || 0}/220 characters</p>
            </div>
            ...
            <div className="rounded-xl border border-gray-200 p-5">
              <div className="flex items-center justify-between mb-3">
                ...
                <button onClick={() => copyField(result.about, 'About')} className="flex items-center gap-1 text-xs text-gray-400 hover:text-gray-600 transition">
                  {copiedField === 'About' ? <CheckCircle className="h-3.5 w-3.5 text-green-600" /> : <Copy className="h-3.5 w-3.5" />} Copy
                </button>
              </div>
              <div className="rounded-lg bg-gray-50 border border-gray-100 p-4 text-sm text-gray-700 leading-relaxed whitespace-pre-wrap">{result.about}</div>
              <p className="mt-1 text-[10px] text-gray-400">{result.about?.length || 0}/2000 characters</p>
            </div>

            {/* Skills */}
            <div className="rounded-xl border border-gray-200 p-5">
              <div className="flex items-center justify-between mb-3">
                <h2 className="text-base font-semibold text-gray-900">Top Skills</h2>
                <button onClick={() => copyField(result.skills?.join(', ') || '', 'Skills')} className="flex items-center gap-1 text-xs text-gray-400 hover:text-gray-600 transition">
                  {copiedField === 'Skills' ? <CheckCircle className="h-3.5 w-3.5 text-green-600" /> : <Copy className="h-3.5 w-3.5" />} Copy
                </button>
              </div>
            ...
            {result.tips?.length > 0 && (
              <div className="rounded-xl border border-amber-200 bg-amber-50/50 p-5">
                <h2 className="text-base font-semibold text-gray-900 flex items-center gap-2"><Lightbulb className="h-5 w-5 text-amber-500" /> Additional Tips</h2>
                <div className="mt-3 space-y-2">
                  {result.tips.map((tip: string, i: number) => (
                    <div key={i} className="flex items-start gap-2 text-sm text-gray-600"><span className="text-amber-500 font-bold">{i + 1}.</span>{tip}</div>
                  ))}
                </div>
              </div>
            )}
