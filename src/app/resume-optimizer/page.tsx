...
export default function ResumeOptimizerPage() {
  ...
  return (
    <div>
      ...
      <p className="mt-1 text-sm text-dark-200">{jt || 'ATS audit + AI rewrite + download.'}</p>
      <div className="mt-6 grid gap-6 lg:grid-cols-5">
        <div className="lg:col-span-2 space-y-4">
          {profile && <div className="rounded-lg border border-dark-500 p-3"><p className="text-xs font-medium text-dark-200 mb-2">Resume</p><div className="flex gap-2"><button onClick={() => { setUseBR(true); setFile(null); }} className={['flex-1 rounded-lg border px-3 py-2 text-xs font-medium transition', useBR?'border-brand-500 bg-indigo-500/10 text-brand-300':'border-dark-500 text-dark-200'].join(' ')}>Base</button><button onClick={() => setUseBR(false)} className={['flex-1 rounded-lg border px-3 py-2 text-xs font-medium transition', !useBR?'border-brand-500 bg-indigo-500/10 text-brand-300':'border-dark-500 text-dark-200'].join(' ')}>Upload</button></div>{useBR && <p className="mt-1 text-[10px] text-green-400"><CheckCircle2 className="h-3 w-3 inline" /> {profile.fileName}</p>}</div>}
          {!useBR && <div {...getRootProps()} className={['flex flex-col items-center rounded-xl border-2 border-dashed p-5 text-center cursor-pointer transition', isDragActive?'border-brand-400 bg-indigo-500/10':file?'border-emerald-500/30 bg-green-900/20':'border-dark-400'].join(' ')}><input {...getInputProps()} />{file?<p className="text-xs text-green-400">{file.name}</p>:<p className="text-xs text-dark-200">Drop resume</p>}</div>}
          <textarea rows={5} value={jd} onChange={e => setJd(e.target.value)} placeholder="Paste full JD..." className="w-full rounded-lg border border-dark-400 bg-dark-700 px-3 py-2 text-sm text-white focus:border-brand-400 focus:outline-none focus:ring-1 focus:ring-brand-400" />
          ...
          {ok && <div className="rounded-xl border-2 border-purple-800/30 bg-purple-900/20/50 p-4 space-y-3">
            ...
            {gen && <>
              {analysis?.overall_score && gen.ats_match_summary?.estimated_score && <div className="rounded-lg bg-dark-800 border border-purple-800/20 p-3 flex items-center gap-3"><div className="text-center"><p className="text-[10px] text-dark-300">Before</p><p className="text-lg font-bold text-amber-400">{analysis.overall_score}</p></div><TrendingUp className="h-5 w-5 text-green-500" /><div className="text-center"><p className="text-[10px] text-dark-300">After</p><p className="text-lg font-bold text-green-400">{gen.ats_match_summary.estimated_score}</p></div><span className="ml-auto rounded-full bg-green-900/20 px-2 py-0.5 text-xs font-bold text-green-400">+{gen.ats_match_summary.estimated_score - analysis.overall_score}</span></div>}
              {gen.jd_analysis && <div className="rounded-lg bg-dark-700 p-2 text-[10px] text-dark-200"><span className="font-medium">Detected:</span> {gen.jd_analysis.company_type} / {gen.jd_analysis.role_level}</div>}
              ...
              <div className="flex gap-2"><button onClick={doSave} disabled={svd} className={['flex-1 rounded-lg border px-3 py-2 text-xs font-medium transition flex items-center justify-center gap-1', svd?'border-green-800/30 bg-green-900/20 text-green-400':'border-dark-500 text-dark-100 hover:bg-dark-700'].join(' ')}>{svd?<CheckCircle2 className="h-3 w-3" />:<Save className="h-3 w-3" />} {svd?'Saved':'Save Version'}</button><button onClick={() => goTo('/cover-letter', { cl_jd: jd, cl_title: jt, cl_company: co })} className="flex-1 rounded-lg border border-green-800/30 bg-green-900/20 px-3 py-2 text-xs font-medium text-green-400 hover:bg-green-900/30 transition flex items-center justify-center gap-1"><FileSignature className="h-3 w-3" /> Cover Letter</button></div>
          ...
          {analysis && gen && <div className="flex gap-2"><button onClick={() => setPanel('audit')} className={['flex-1 rounded-lg border px-3 py-1.5 text-xs font-medium transition', panel==='audit'?'border-brand-500 bg-indigo-500/10 text-brand-300':'border-dark-500 text-dark-200'].join(' ')}>Audit</button><button onClick={() => setPanel('preview')} className={['flex-1 rounded-lg border px-3 py-1.5 text-xs font-medium transition', panel==='preview'?'border-violet-500/30 bg-purple-900/20 text-purple-400':'border-dark-500 text-dark-200'].join(' ')}>Preview</button></div>}
        ...
        <div className="lg:col-span-3 rounded-xl border border-dark-500 p-5 min-h-[600px] overflow-y-auto max-h-[calc(100vh-120px)]">
          {panel==='preview' && gen ? (
            <div className="space-y-3">
              <div className="flex items-center justify-between"><h2 className="text-sm font-semibold text-purple-400">Optimized Resume</h2>{gen.ats_match_summary && <span className="rounded-full bg-green-900/30 text-green-400 px-3 py-1 text-xs font-bold">{gen.ats_match_summary.estimated_score}%</span>}{gen.page_count && <span className="text-[10px] text-dark-300">{gen.page_count}pg</span>}</div>
              <div className="text-center border-b pb-3"><h1 className="text-xl font-bold text-white">{gen.name}</h1><p className="text-[10px] text-dark-200 mt-1">{[gen.email,gen.phone,gen.location,gen.linkedin,gen.github].filter(Boolean).join(' | ')}</p></div>
              {gen.summary && <div><h3 className="text-[10px] font-bold text-dark-100 uppercase tracking-wide border-b border-dark-500 pb-1 mb-1">Summary</h3><p className="text-xs text-dark-100 leading-relaxed">{gen.summary}</p></div>}
              {gen.skills_grouped && Object.keys(gen.skills_grouped).length > 0 ? (
                <div><h3 className="text-[10px] font-bold text-dark-100 uppercase tracking-wide border-b border-dark-500 pb-1 mb-1">Technical Skills</h3>{Object.entries(gen.skills_grouped).map(([c,s]) => Array.isArray(s) && s.length > 0 ? <p key={c} className="text-xs text-dark-100 mb-0.5"><span className="font-semibold">{c}:</span> {(s as string[]).join(', ')}</p> : null)}</div>
              ) : gen.skills?.length > 0 && <div><h3 className="text-[10px] font-bold text-dark-100 uppercase tracking-wide border-b border-dark-500 pb-1 mb-1">Skills</h3><p className="text-xs text-dark-100">{gen.skills.join(' • ')}</p></div>}
              {gen.experience?.map((e: any, i: number) => (<div key={i}>{i===0 && <h3 className="text-[10px] font-bold text-dark-100 uppercase tracking-wide border-b border-dark-500 pb-1 mb-1">Experience</h3>}<div className="mb-3"><div className="flex justify-between"><p className="text-xs font-bold text-dark-50">{e.company}</p><p className="text-[10px] text-dark-300">{e.dates}</p></div><p className="text-xs italic text-dark-100">{e.title}</p><ul className="mt-1 space-y-0.5">{e.bullets?.map((b:string,j:number) => <li key={j} className="text-xs text-dark-100 pl-3 relative before:content-['-'] before:absolute before:left-0 before:text-dark-300">{b}</li>)}</ul></div></div>))}
              {gen.education?.map((e: any, i: number) => (<div key={i}>{i===0 && <h3 className="text-[10px] font-bold text-dark-100 uppercase tracking-wide border-b border-dark-500 pb-1 mb-1">Education</h3>}<p className="text-xs font-bold text-dark-50">{e.institution}</p><p className="text-xs text-dark-100">{e.degree}</p></div>))}
              {gen.certifications?.filter(Boolean).length>0 && <div><h3 className="text-[10px] font-bold text-dark-100 uppercase tracking-wide border-b border-dark-500 pb-1 mb-1">Certifications</h3>{gen.certifications.filter(Boolean).map((c:string,i:number) => <p key={i} className="text-xs text-dark-100">- {c}</p>)}</div>}
            ...
            <div className="flex h-full items-center justify-center text-sm text-dark-300">{aing?<div className="text-center"><Loader2 className="h-8 w-8 animate-spin text-brand-400 mx-auto" /><p className="mt-3">Analyzing...</p></div>:ging?<div className="text-center"><Loader2 className="h-8 w-8 animate-spin text-purple-400 mx-auto" /><p className="mt-3">Optimizing...</p></div>:'Run audit or generate.'}</div>
          ) : (
            <div className="space-y-4">
              ...
              <div className="flex gap-1 border-b">{[{k:'overview',l:'Keywords'},{k:'categories',l:'Scores'},{k:'fixes',l:'Fixes',c:analysis.priority_fixes?.length},{k:'bullets',l:'Rewrites',c:analysis.weak_bullets?.length}].map((t:any) => <button key={t.k} onClick={() => setTab(t.k)} className={['px-3 py-1.5 text-xs font-medium border-b-2 transition', tab===t.k?'border-brand-600 text-brand-300':'border-transparent text-dark-300'].join(' ')}>{t.l}{t.c?<span className="ml-1 text-[9px] bg-red-900/20 text-red-400 rounded-full px-1">{t.c}</span>:null}</button>)}</div>
              <div className="overflow-y-auto max-h-[420px]">
                {tab==='overview' && analysis.keyword_match && <div className="space-y-3"><div><h3 className="text-sm font-semibold text-dark-100">Keywords ({analysis.keyword_match.match_percentage}%)</h3><div className="mt-1 h-2 rounded-full bg-dark-600"><div className={bar(analysis.keyword_match.match_percentage)} style={{width:`${analysis.keyword_match.match_percentage}%`,height:'100%',borderRadius:'9999px'}} /></div></div><div><p className="text-xs font-medium text-green-400 mb-1">Matched</p><div className="flex flex-wrap gap-1">{analysis.keyword_match.matched?.map((k:string) => <span key={k} className="inline-flex items-center gap-0.5 rounded-full bg-green-900/20 px-2 py-0.5 text-[10px] text-green-400"><CheckCircle2 className="h-2.5 w-2.5" />{k}</span>)}</div></div><div><p className="text-xs font-medium text-red-400 mb-1">Missing</p><div className="flex flex-wrap gap-1">{analysis.keyword_match.missing?.map((k:string) => <span key={k} className="inline-flex items-center gap-0.5 rounded-full bg-red-900/20 px-2 py-0.5 text-[10px] text-red-400"><AlertTriangle className="h-2.5 w-2.5" />{k}</span>)}</div></div></div>}
                {tab==='categories' && analysis.category_scores?.map((c:any) => <div key={c.category} className="rounded-lg border border-dark-500 p-3 mb-2"><div className="flex items-center justify-between"><span className="text-xs font-medium text-dark-50">{c.category}</span><span className={['text-sm font-bold',c.score>=8?'text-green-400':c.score>=6?'text-amber-400':'text-red-400'].join(' ')}>{c.score}/10</span></div><div className="mt-1 h-1 rounded-full bg-dark-600"><div className={bar(c.score,10)} style={{width:`${c.score*10}%`,height:'100%',borderRadius:'9999px'}} /></div><p className="mt-1 text-[10px] text-dark-200">{c.explanation}</p>{c.fixes?.map((f:string,i:number) => <p key={i} className="text-[10px] text-dark-100 mt-0.5">→ {f}</p>)}</div>)}
                {tab==='fixes' && analysis.priority_fixes?.map((f:any) => <div key={f.rank} className="rounded-lg border border-dark-500 p-3 mb-2 flex gap-2"><div className="flex h-5 w-5 items-center justify-center rounded-full bg-red-900/20 text-[10px] font-bold text-red-400">{f.rank}</div><div><p className="text-xs font-semibold text-dark-50">{f.section}</p><p className="text-[10px] text-dark-100">{f.action}</p></div></div>)}
                {tab==='bullets' && analysis.weak_bullets?.map((b:any,i:number) => <div key={i} className="rounded-lg border border-dark-500 p-3 mb-2"><p className="text-[9px] text-red-400 uppercase font-medium">Original</p><p className="text-xs text-dark-100">{b.original}</p><p className="text-[9px] text-green-500 uppercase font-medium mt-1">Rewritten</p><p className="text-xs text-dark-50 font-medium">{b.rewritten}</p></div>)}
              </div>
            </div>
          )}
        </div>
