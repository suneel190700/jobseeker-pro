...
export default function InterviewPrepPage() {
  ...
  return (
    <div className="max-w-4xl">
      ...
      <p className="mt-1 text-sm text-dark-200">AI-generated questions, STAR stories, and company research for your interview.</p>
      ...
      <div className="mt-6 grid grid-cols-2 gap-3">
        <div><label className="block text-xs font-medium text-dark-100 mb-1">Job Title</label><input type="text" value={jobTitle} onChange={(e) => setJobTitle(e.target.value)} placeholder="e.g. AI Engineer" className="w-full rounded-lg border border-dark-400 bg-dark-700 px-3 py-2 text-sm text-white focus:border-brand-400 focus:outline-none focus:ring-1 focus:ring-brand-400" /></div>
        <div><label className="block text-xs font-medium text-dark-100 mb-1">Company</label><input type="text" value={companyName} onChange={(e) => setCompanyName(e.target.value)} placeholder="e.g. Google" className="w-full rounded-lg border border-dark-400 bg-dark-700 px-3 py-2 text-sm text-white focus:border-brand-400 focus:outline-none focus:ring-1 focus:ring-brand-400" /></div>
      </div>
      <div className="mt-3"><label className="block text-xs font-medium text-dark-100 mb-1">Job Description</label><textarea rows={5} value={jobDescription} onChange={(e) => setJobDescription(e.target.value)} placeholder="Paste JD..." className="w-full rounded-lg border border-dark-400 bg-dark-700 px-3 py-2 text-sm text-white focus:border-brand-400 focus:outline-none focus:ring-1 focus:ring-brand-400" /></div>

      {/* Tabs */}
      <div className="mt-6 flex gap-2 border-b border-dark-500 pb-0">
        {[{ key: 'questions', label: 'Interview Questions', icon: MessageCircle }, { key: 'star', label: 'STAR Stories', icon: BookOpen }, { key: 'company', label: 'Company Brief', icon: Building2 }].map((t) => (
          <button key={t.key} onClick={() => { setActiveTab(t.key as any); if (!data[t.key]) generate(t.key); }}
            className={['flex items-center gap-1.5 px-4 py-2.5 text-sm font-medium border-b-2 transition', activeTab === t.key ? 'border-brand-600 text-brand-300' : 'border-transparent text-dark-300 hover:text-dark-100'].join(' ')}>
      ...
      <div className="mt-6">
        {loading && <div className="flex items-center justify-center py-12"><Loader2 className="h-6 w-6 animate-spin text-brand-400" /></div>}
            ...
            <div className="space-y-6">
              {['behavioral', 'technical', 'system_design', 'role_specific'].map((cat) => (
                questions[cat]?.length > 0 && (
                  <div key={cat}>
                    <h3 className="text-sm font-semibold text-dark-50 capitalize mb-3">{cat.replace('_', ' ')} Questions</h3>
                    <div className="space-y-3">{questions[cat].map((q: any, i: number) => (
                      <div key={i} className="rounded-lg border border-dark-500 p-4">
                        <p className="text-sm font-medium text-white">{q.question}</p>
                        <p className="text-xs text-dark-300 mt-1">Why: {q.why_asked}</p>
        ...
        {!loading && activeTab === 'star' && (
          !star ? (
            ...
            <div className="space-y-4">{star.stories?.map((s: any, i: number) => (
              <div key={i} className="rounded-lg border border-dark-500 p-4">
                <div className="flex items-center justify-between mb-2"><h4 className="text-sm font-semibold text-white">{s.title}</h4><span className="text-[10px] text-dark-300">For: {s.relevant_for}</span></div>
                <div className="grid grid-cols-2 gap-3 text-xs">
                  <div><p className="font-medium text-blue-400">Situation</p><p className="text-dark-100 mt-0.5">{s.situation}</p></div>
                  <div><p className="font-medium text-amber-400">Task</p><p className="text-dark-100 mt-0.5">{s.task}</p></div>
                  <div><p className="font-medium text-green-400">Action</p><p className="text-dark-100 mt-0.5">{s.action}</p></div>
                  <div><p className="font-medium text-purple-400">Result</p><p className="text-dark-100 mt-0.5">{s.result}</p></div>
                </div>
                {s.keywords?.length > 0 && <div className="mt-2 flex flex-wrap gap-1">{s.keywords.map((k: string) => (<span key={k} className="rounded-full bg-dark-600 px-2 py-0.5 text-[9px] text-dark-100">{k}</span>))}</div>}
              </div>
            ))}</div>
          )
        )}
        ...
        {!loading && activeTab === 'company' && (
          !company ? (
            ...
            <div className="space-y-4">
              ...
              {company.culture_values?.length > 0 && <div className="rounded-lg border border-dark-500 p-4"><h4 className="text-sm font-semibold text-dark-50">Culture & Values</h4><div className="mt-2 flex flex-wrap gap-2">{company.culture_values.map((v: string) => (<span key={v} className="rounded-full bg-purple-900/20 px-3 py-1 text-xs text-purple-400">{v}</span>))}</div></div>}
              {company.interview_talking_points?.length > 0 && <div className="rounded-lg border border-dark-500 p-4"><h4 className="text-sm font-semibold text-dark-50">Talking Points</h4><div className="mt-2 space-y-1">{company.interview_talking_points.map((t: string, i: number) => (<p key={i} className="text-xs text-dark-100">• {t}</p>))}</div></div>}
              ...
              {company.recent_news && <div className="rounded-lg border border-dark-500 p-4"><h4 className="text-sm font-semibold text-dark-50">Research Before Interview</h4><p className="text-xs text-dark-100 mt-1">{company.recent_news}</p></div>}
            </div>
          )
        )}
      </div>
