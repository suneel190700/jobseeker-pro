...
export default function InterviewPrepPage() {
  ...
  return (
    <div className="max-w-4xl">
      ...
      <p className="mt-1 text-sm text-gray-500">AI-generated questions, STAR stories, and company research for your interview.</p>
      ...
      <div className="mt-6 grid grid-cols-2 gap-3">
        <div><label className="block text-xs font-medium text-gray-700 mb-1">Job Title</label><input type="text" value={jobTitle} onChange={(e) => setJobTitle(e.target.value)} placeholder="e.g. AI Engineer" className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-brand-500 focus:outline-none focus:ring-1 focus:ring-brand-500" /></div>
        <div><label className="block text-xs font-medium text-gray-700 mb-1">Company</label><input type="text" value={companyName} onChange={(e) => setCompanyName(e.target.value)} placeholder="e.g. Google" className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-brand-500 focus:outline-none focus:ring-1 focus:ring-brand-500" /></div>
      </div>
      <div className="mt-3"><label className="block text-xs font-medium text-gray-700 mb-1">Job Description</label><textarea rows={5} value={jobDescription} onChange={(e) => setJobDescription(e.target.value)} placeholder="Paste JD..." className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-brand-500 focus:outline-none focus:ring-1 focus:ring-brand-500" /></div>

      {/* Tabs */}
      <div className="mt-6 flex gap-2 border-b border-gray-200 pb-0">
        {[{ key: 'questions', label: 'Interview Questions', icon: MessageCircle }, { key: 'star', label: 'STAR Stories', icon: BookOpen }, { key: 'company', label: 'Company Brief', icon: Building2 }].map((t) => (
          <button key={t.key} onClick={() => { setActiveTab(t.key as any); if (!data[t.key]) generate(t.key); }}
            className={['flex items-center gap-1.5 px-4 py-2.5 text-sm font-medium border-b-2 transition', activeTab === t.key ? 'border-brand-600 text-brand-700' : 'border-transparent text-gray-400 hover:text-gray-600'].join(' ')}>
            <t.icon className="h-4 w-4" />{t.label}
            {data[t.key] && <CheckCircle className="h-3 w-3 text-green-600" />}
          </button>
        ))}
      </div>
            ...
            <div className="space-y-6">
              {['behavioral', 'technical', 'system_design', 'role_specific'].map((cat) => (
                questions[cat]?.length > 0 && (
                  <div key={cat}>
                    <h3 className="text-sm font-semibold text-gray-800 capitalize mb-3">{cat.replace('_', ' ')} Questions</h3>
                    <div className="space-y-3">{questions[cat].map((q: any, i: number) => (
                      <div key={i} className="rounded-lg border border-gray-100 p-4">
                        <p className="text-sm font-medium text-gray-900">{q.question}</p>
                        <p className="text-xs text-gray-400 mt-1">Why: {q.why_asked}</p>
          ...
          !star ? (
            ...
            <div className="space-y-4">{star.stories?.map((s: any, i: number) => (
              <div key={i} className="rounded-lg border border-gray-200 p-4">
                <div className="flex items-center justify-between mb-2"><h4 className="text-sm font-semibold text-gray-900">{s.title}</h4><span className="text-[10px] text-gray-400">For: {s.relevant_for}</span></div>
                <div className="grid grid-cols-2 gap-3 text-xs">
                  <div><p className="font-medium text-blue-600">Situation</p><p className="text-gray-600 mt-0.5">{s.situation}</p></div>
                  <div><p className="font-medium text-amber-600">Task</p><p className="text-gray-600 mt-0.5">{s.task}</p></div>
                  <div><p className="font-medium text-green-600">Action</p><p className="text-gray-600 mt-0.5">{s.action}</p></div>
                  <div><p className="font-medium text-purple-600">Result</p><p className="text-gray-600 mt-0.5">{s.result}</p></div>
                </div>
                {s.keywords?.length > 0 && <div className="mt-2 flex flex-wrap gap-1">{s.keywords.map((k: string) => (<span key={k} className="rounded-full bg-gray-100 px-2 py-0.5 text-[9px] text-gray-600">{k}</span>))}</div>}
              </div>
            ))}</div>
          )
        )}
        ...
        {!loading && activeTab === 'company' && (
          !company ? (
            ...
            <div className="space-y-4">
              ...
              {company.culture_values?.length > 0 && <div className="rounded-lg border border-gray-200 p-4"><h4 className="text-sm font-semibold text-gray-800">Culture & Values</h4><div className="mt-2 flex flex-wrap gap-2">{company.culture_values.map((v: string) => (<span key={v} className="rounded-full bg-purple-50 px-3 py-1 text-xs text-purple-700">{v}</span>))}</div></div>}
              {company.interview_talking_points?.length > 0 && <div className="rounded-lg border border-gray-200 p-4"><h4 className="text-sm font-semibold text-gray-800">Talking Points</h4><div className="mt-2 space-y-1">{company.interview_talking_points.map((t: string, i: number) => (<p key={i} className="text-xs text-gray-600">• {t}</p>))}</div></div>}
              ...
              {company.recent_news && <div className="rounded-lg border border-gray-200 p-4"><h4 className="text-sm font-semibold text-gray-800">Research Before Interview</h4><p className="text-xs text-gray-600 mt-1">{company.recent_news}</p></div>}
            </div>
          )
        )}
      </div>
