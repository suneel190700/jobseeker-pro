...
export default function LoginPage() {
  ...
  return(<div className="min-h-screen bg-[#0a0a0a] flex items-center justify-center px-4"><div className="w-full max-w-sm">
    <div className="text-center mb-8"><div className="inline-flex h-10 w-10 rounded-xl items-center justify-center bg-gradient-to-br from-emerald-400 to-emerald-600 mb-4"><Sparkles className="h-5 w-5 text-white" /></div><h1 className="text-xl font-bold text-white">Welcome back</h1><p className="mt-1 text-sm text-zinc-500">Sign in to continue</p></div>
    <div className="rounded-xl border border-white/5 bg-white/[0.02] p-6">
      <form onSubmit={handle} className="space-y-3.5">
        <div><label className="label">Email</label><input type="email" value={email} onChange={e=>setEmail(e.target.value)} required className="inp" placeholder="you@email.com" /></div>
      ...
      <div className="mt-3"><button onClick={google} className="btn-s w-full py-2.5">Continue with Google</button></div>
    </div>
    <p className="mt-5 text-center text-sm text-zinc-600">No account? <Link href="/auth/signup" className="text-emerald-400 font-medium hover:text-emerald-300">Sign up</Link></p>
  </div></div>);
}
