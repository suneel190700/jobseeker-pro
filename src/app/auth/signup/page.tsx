...
export default function SignupPage() {
  ...
  if(done)return(<div className="min-h-screen bg-[#0a1628] flex items-center justify-center px-4"><div className="rounded-xl border border-white/5 bg-white/[0.02] p-6 text-center max-w-sm"><Sparkles className="h-8 w-8 text-emerald-400 mx-auto mb-3" /><h1 className="text-lg font-bold text-white">Check your email</h1><p className="mt-1 text-sm text-slate-500">Confirmation sent to {email}</p><Link href="/auth/login" className="mt-3 inline-block text-sm text-emerald-400">Back to sign in</Link></div></div>);
  return(<div className="min-h-screen bg-[#0a1628] flex items-center justify-center px-4"><div className="w-full max-w-sm">
    <div className="text-center mb-8"><div className="inline-flex h-10 w-10 rounded-xl items-center justify-center bg-gradient-to-br from-emerald-400 to-emerald-600 mb-4"><Sparkles className="h-5 w-5 text-white" /></div><h1 className="text-xl font-bold text-white">Create account</h1><p className="mt-1 text-sm text-slate-500">Start your job search</p></div>
    <div className="rounded-xl border border-white/5 bg-white/[0.02] p-6">
      <form onSubmit={handle} className="space-y-3.5">
        <div><label className="label">Full Name</label><input type="text" value={name} onChange={e=>setName(e.target.value)} required className="input-dark" /></div>
        <div><label className="label">Email</label><input type="email" value={email} onChange={e=>setEmail(e.target.value)} required className="input-dark" /></div>
        <div><label className="label">Password</label><input type="password" value={pw} onChange={e=>setPw(e.target.value)} required minLength={6} className="input-dark" /></div>
    ...
    <p className="mt-5 text-center text-sm text-slate-600">Have an account? <Link href="/auth/login" className="text-emerald-400 font-medium hover:text-emerald-300">Sign in</Link></p>
  </div></div>);
}
